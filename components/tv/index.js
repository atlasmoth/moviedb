...
export default function Movie(props) {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/tv/${props.data.id}/credits?api_key=${process.env.NEXT_PUBLIC_API_KEY}`
    )
      .then((res) => res.json())
      .then(({ cast }) => {
        cast = cast.filter(
    ...
    // get recommendations
    fetch(
      `https://api.themoviedb.org/3/tv/${props.data.id}/recommendations?api_key=${process.env.NEXT_PUBLIC_API_KEY}`
    )
      .then((res) => res.json())
      .then(({ results }) => {
